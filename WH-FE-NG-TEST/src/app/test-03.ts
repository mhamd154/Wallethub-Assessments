...
export class Test03Component {
  ...
  isValidPassowrd(password: string) {
    const validPasswordPattern =
      /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/;
    const isValidPassowrd = validPasswordPattern.test(password);
    return isValidPassowrd;
  }

  validateField(email, password) {
    const emailAndPasswordValid =
      this.isValidEmail(email) && this.isValidPassowrd(password);
    return emailAndPasswordValid;
  }

  submitForm(event) {
    event.preventDefault();
    this.logged_in = this.validateField(this.email, this.password);
  }
}
