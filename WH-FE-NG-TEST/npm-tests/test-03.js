...
for (var i = 1; i < arr.length; i += 2) {
  finalObj[arr[i - 1].key] = arr[i].age;
}
console.log(`There are ${count} items with age equal to 32`);
const str = JSON.stringify(finalObj).slice(1, -1);
let nstr = str.replace(/,+/g, `,\n`);
console.log(nstr);
fs.writeFile(FILE1, nstr);
// write code here
